...
from application.models.models import Lessons, LessonSchema, Questionnaire, QuestionnaireSchema,Advertisements, AdSchema
...
class APILessonView(FlaskView):
		...
		if sentences.count() > 0:
			...
			for s in sentences:
				if s.is_straight_translation == 1 or s.is_multiple_images:
					lesson = SimpleSentenceLessonBuilder(s)\
						.split_into_words()\
						.create_dropDown()\
						.build()
	...
	def group_lessons(self, group_id):
		...
		ads = Advertisements.query.filter(user.age >= Advertisements.ad_age,Advertisements.ad_gender==user.gender).all()
		if sentences.count() > 0:
			sentences = sentences.all()
			for s in sentences:
				if s.is_straight_translation == 1 or s.is_multiple_images:
